...
import { sendPropertyInquiry } from '@/lib/emailjs'
...
export function PropertyInquiryModal({ isOpen, onClose, slug, titulo }: PropertyInquiryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await sendPropertyInquiry({ ...formData, slug })
      setSuccess(true)
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-0 z-40 bg-slate-950/60 backdrop-blur-sm animate-backdrop-in"
        onClick={onClose}
      />
        ...
        <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-0 shadow-2xl shadow-[0_8px_40px_-8px_rgba(3,17,62,0.45)] animate-modal-in">
          ...
          <div className="relative bg-[#0A43D8] px-8 py-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-extrabold text-white">Solicitar Informações</h2>
                <p className="mt-0.5 text-sm font-medium text-blue-100 line-clamp-1">{titulo}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-4">
            <div>
              ...
              <input type="text" name="nome" value={formData.nome} onChange={handleChange} required className={inputClass} placeholder="Seu nome completo" />
            </div>
            ...
            <div>
              ...
              <textarea name="mensagem" value={formData.mensagem} onChange={handleChange} rows={3} className={`${inputClass} resize-none`} placeholder="Escreva a sua mensagem..." />
            </div>

            {/* Feedback */}
            {error && (
              <div className="flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 p-3.5 text-sm font-medium text-red-600 animate-in fade-in slide-in-from-top-2 duration-300">
                <AlertCircle size={16} className="flex-shrink-0" />
                {error}
              </div>
            )}
            {success && (
              <div className="flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 p-3.5 text-sm font-medium text-green-600 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="submit" disabled={loading} className="flex-1 rounded-xl bg-[#0A43D8] py-3 text-sm font-bold text-white shadow-lg shadow-blue-900/25 transition-all hover:bg-[#021a5c] hover:shadow-xl hover:shadow-blue-900/30 disabled:opacity-50 cursor-pointer">
